        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          ...
          <div className="text-center mb-20">
            ...
            <div className="flex flex-wrap justify-center gap-6 mb-8">
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-300 mb-1">{commandCategories.length}</div>
                <div className="text-xs text-gray-400 font-medium">Categories</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-purple-300 mb-1">{totalCommands}</div>
                <div className="text-xs text-gray-400 font-medium">Total Commands</div>
              </div>
              <div className="text-center">
          ...
          <div className="mb-12">
            <div className="relative max-w-xl mx-auto">
              ...
              <div className="relative glass-border rounded-xl overflow-hidden backdrop-blur-xl">
                <div className="flex items-center px-4 py-3">
                  ...
                  />
                  {searchTerm && (
                    <div className="ml-3 px-2 py-1 rounded-full bg-blue-500/20 text-blue-400 text-xs font-medium">
          ...
          <div className="space-y-8">
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    className="w-full p-6 flex items-center justify-between hover:bg-white/5 transition-all duration-300 group/header"
                  >
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        <div className="p-3 rounded-xl bg-gradient-to-br from-blue-500/10 to-purple-500/10 group-hover/header:scale-110 group-hover/header:rotate-6 transition-all duration-500 shadow-lg border border-white/10">
                          <Icon className="w-6 h-6 text-white drop-shadow-lg" />
                        </div>
                        <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-blue-500/20 to-purple-500/20 opacity-0 group-hover/header:opacity-100 transition-opacity duration-300 blur-xl" />
                      </div>
                      <div className="text-left">
                        <h3 className="text-xl font-bold mb-2 group-hover/header:text-white transition-colors duration-300">
                          {category.name}
                        </h3>
                        <p className="text-gray-400 group-hover/header:text-gray-300 transition-colors duration-300 text-base">
                          {category.description}
                        </p>
                        <div className="flex items-center gap-3 mt-2">
                          <span className="px-2 py-1 rounded-full bg-white/5 text-xs font-medium text-blue-400">
                    ...
                    <div className="flex items-center gap-4">
                      <ArrowRight className={`w-5 h-5 text-gray-400 transition-all duration-300 ${isExpanded ? 'rotate-90' : 'group-hover/header:translate-x-1'}`} />
                      {isExpanded ? (
                        <ChevronUp className="w-5 h-5 text-gray-400 group-hover/header:text-white transition-colors" />
                      ) : (
                        <ChevronDown className="w-5 h-5 text-gray-400 group-hover/header:text-white transition-colors" />
                      )}
                    </div>
                  </button>
                      ...
                      <div className="grid gap-3 p-6">
                        {visibleCategoryCommands.map((command, commandIndex) => (
                          <div
                            key={command.name}
                            id={command.name}
                            className="group/command p-4 rounded-xl bg-white/[0.03] hover:bg-white/[0.08] transition-all duration-300 border border-white/5 hover:border-white/10 hover:scale-[1.01]"
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="flex items-start justify-between">
                                <div className="flex items-center gap-3">
                                  <div className="p-1.5 rounded-lg bg-gradient-to-br from-blue-500/10 to-purple-500/10 group-hover/command:scale-110 transition-transform duration-300">
                                    <Bot className="w-4 h-4 text-blue-400" />
                                  </div>
                                  <div>
                                    <code className="text-base font-mono text-blue-400 font-semibold group-hover/command:text-blue-300 transition-colors">
                                      {command.name}
                                    </code>
                                    {command.permissions && (
                                      <div className="flex items-center gap-2 mt-1">
                                        ...
                                        <div className="flex flex-wrap gap-1">
                                          {command.permissions.map((perm, i) => (
                                            <span key={i} className="px-1.5 py-0.5 rounded-full bg-yellow-500/10 text-yellow-400 text-xs font-medium border border-yellow-500/20">
                                              {perm}
                                            </span>
                                          ))}
                                        </div>
                                ...
                                <Sparkles className="w-4 h-4 text-purple-400 opacity-0 group-hover/command:opacity-100 transition-opacity duration-300" />
                              </div>

                              {/* Command Description */}
                              <p className="text-sm text-gray-300 group-hover/command:text-white transition-colors duration-300 leading-relaxed">
                                {command.description}
                              </p>
                              ...
                              <div className="space-y-2">
                                <div>
                                  <span className="text-xs font-semibold text-gray-400 mb-1 block">Usage:</span>
                                  <code className="text-gray-300 bg-black/20 px-2 py-1 rounded text-xs font-mono border border-white/10 block">
                                    {command.usage}
                                  </code>
                                </div>
                                {command.examples && (
                                  <div>
                                    <span className="text-xs font-semibold text-gray-400 mb-1 block">Examples:</span>
                                    <div className="flex flex-wrap gap-2">
                                      {command.examples.map((example, i) => (
                                        <code key={i} className="text-gray-300 bg-black/20 px-2 py-1 rounded text-xs font-mono border border-white/10 hover:bg-black/30 transition-colors">
                                          {example}
                                        </code>
                                      ))}
                                    </div>
            ...
            {commandCategories.every(category => 
              ...
              <div className="text-center py-12">
                <div className="glass-border rounded-2xl p-8 max-w-xl mx-auto">
                  <div className="relative mb-8">
                    ...
                    <div className="relative inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-blue-500/10 to-purple-500/10 animate-float">
                      <Search className="w-8 h-8 text-gray-400" />
                    </div>
                  </div>
                  <h3 className="text-xl font-bold mb-3">No Commands Found</h3>
                  <p className="text-gray-400 mb-8">
                    We couldn't find any commands matching "{searchTerm}". Try adjusting your search terms.
                  </p>
                  <button
                    onClick={() => setSearchTerm('')}
                    className="px-4 py-2 bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-lg border border-white/10 hover:bg-white/10 transition-all duration-300 font-medium text-sm"
                  >
                    Clear Search
                  </button>
                </div>
          ...
          <div className="mt-12 text-center">
            <div className="glass-border rounded-2xl p-8 relative overflow-hidden group hover:scale-[1.01] transition-all duration-300">
              ...
              <div className="relative z-10">
                <Star className="w-10 h-10 text-yellow-400 mx-auto mb-4 animate-pulse" />
